...
export function BaroSolicitudPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    enabled: puedeGestionar && Boolean(sedeId),
  });
    ...
    <div className="flex-1 overflow-y-auto bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center gap-3 sticky top-0 z-10">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-500">Médicos que atienden baropodometría por sede, solo cuando el paciente los pide.</p>
        </div>
        <select className="input text-sm" value={sedeId} onChange={e => setSedeId(e.target.value)}>
          {sedes.map((s: { id: string; nombre: string }) => <option key={s.id} value={s.id}>{s.nombre}</option>)}
        </select>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-rose-50 border border-rose-100 rounded-xl p-4 text-xs text-rose-900 leading-relaxed">
          El registro de médico de baro es <b>por sede</b>: solo se listan profesionales que trabajan en la sede elegida. La baropodometría se
          asigna <b>automáticamente</b> a una máquina; quienes estén aquí aparecen como opción de médico al reservar en esa sede.
        </div>

        {/* Agregar */}
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Agregar a la lista (de esta sede)</p>
        ...
        <div className="space-y-2">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest px-1">
            En esta sede {data && <span className="text-slate-400">· {data.porSolicitud.length}</span>}
          </p>
          {isLoading ? (
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6 text-center text-slate-400 text-sm">Ningún médico atiende baro en esta sede todavía.</div>
          ) : (
            data!.porSolicitud.map((p) => (
              <div key={p.id} className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xxs text-slate-400">{TIPO_LABEL[p.tipo] ?? p.tipo}</p>
                </div>
                <button
          ...
          <p className="text-xxs text-slate-400 px-1 pt-1">Al quitar a alguien de la última sede, se desactiva su atención de baropodometría (no se borra el historial de citas pasadas).</p>
        </div>
      </div>
    </div>
  );
