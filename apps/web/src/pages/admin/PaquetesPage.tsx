    ...
    <div className="flex flex-col h-full overflow-hidden relative bg-background">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-headline-md text-base font-bold text-on-surface">Paquetes de sesiones</h3>
        ...
        {isLoading ? (
          ...
          <div className="bg-surface-container-lowest rounded-xl border border-dashed border-outline-variant p-10 text-center text-on-surface-variant max-w-5xl">
            ...
            <p className="font-medium text-slate-800 text-sm">Sin paquetes configurados</p>
            <p className="text-xs mt-1 text-slate-500">Crea el primer paquete con el botón de arriba</p>
          </div>
        ) : (
          <div className="space-y-2.5 max-w-5xl">
            {(paquetes ?? []).map(p => (
              <div
                key={p.id}
                className="bg-surface-container-lowest rounded-xl px-4 py-2.5 shadow-2xs border border-slate-200 flex items-center justify-between hover:border-slate-300 transition-colors group"
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <h4 className="font-semibold text-slate-800 text-[13px] truncate leading-tight">{p.nombre}</h4>
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap text-xxs text-slate-500">
                      ...
                      {p.consumeNoShow && (
                        <span className="text-xxs px-1.5 py-0.2 bg-amber-100 text-amber-700 rounded-full font-medium">cuenta no-shows</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0 opacity-90 group-hover:opacity-100 transition-opacity ml-3">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 border border-red-200 rounded-md text-xs font-medium text-red-600 hover:bg-red-50 transition-colors"
                  >
                    Eliminar
                  </button>
                </div>
