...
// Pequeño diálogo reutilizable para pedir un MOTIVO (anular receta, etc.). Mín. 5 caracteres, salvo
// `opcional` (revocar un consentimiento: el paciente no está obligado a expresar el motivo).
export function DialogoMotivo({ titulo, descripcion, confirmar = 'Confirmar', pending, onConfirmar, onClose, opcional = false }: {
  titulo: string; descripcion?: string; confirmar?: string; pending?: boolean; onConfirmar: (motivo: string) => void; onClose: () => void; opcional?: boolean;
}) {
  const [motivo, setMotivo] = useState('');
  const ok = opcional || motivo.trim().length >= 5;
  return (
    <div className="fixed inset-0 bg-inverse-surface/40 backdrop-blur-[2px] z-[130] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-surface-container-lowest w-full max-w-[440px] rounded-2xl overflow-hidden custom-shadow animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
        <div className="p-6 space-y-3">
          ...
          <textarea value={motivo} onChange={(e) => setMotivo(e.target.value)} rows={3} autoFocus maxLength={500} placeholder={opcional ? 'Motivo (opcional: no está obligado a expresarlo)' : 'Motivo (mínimo 5 caracteres)'}
